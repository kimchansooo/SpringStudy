	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$('#selectbtn').click(function(){
				const empno = Number($('#selectnum').val());
				$.ajax({
					type:"get",
					dataType: 'json',
					url  : "${root}/kosa/emp/"+empno,
					success : function(data){ 
						console.log(data);
			...
			$(document).on("click",".delete",function(){
				//console.log(JSON.stringify({"empno" : Number($(this).attr("value2"))}));
				const empno = JSON.stringify(Number($(this).attr("value2")))
				$.ajax({
					type : "DELETE",
					dataType: 'json',
					url  : "${root}/kosa/emp/"+empno,
		...
		function empupdateok(me){
			var tr = $(me).closest('tr');
			var data = JSON.stringify({"empno":Number(tr.find("td:eq(0)").children().val()),
						"ename":tr.find("td:eq(1)").children().val(),
						"sal":Number(tr.find("td:eq(2)").children().val()),
					   });
			$.ajax({
				type : "PUT",
				dataType: 'json',
		...
		<div class="container-fluid">
			<div class="row">
				...
				<fieldset>
					...
					사번 검색: <input type="text" id="selectnum">
